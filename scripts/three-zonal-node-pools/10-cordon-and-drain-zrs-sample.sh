...
echo "Retrieving the name of the pod with app=[zrs-nginx] label..."
podName=$(kubectl get pods -l app=zrs-nginx -n $namespace -o jsonpath='{.items[*].metadata.name}')

if [ -n "$podName" ]; then
  echo "Successfully retrieved the [$podName] pod name for the [zrs-nginx] app label"
else
  echo "Failed to retrieve the pod name for the [zrs-nginx] app label"
  exit 1
fi

# Retrieve the name of the node running the pod
nodeName=$(kubectl get pods -l app=zrs-nginx -n $namespace -o jsonpath='{.items[*].spec.nodeName}')
